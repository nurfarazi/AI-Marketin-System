...
```bash
curl -X POST http://localhost:3000/api/v1/projects/<PROJECT_ID>/ingestions \
  -H "content-type: application/json" \
  -d '{
    "type": "text",
    "payload": {
...
```bash
curl -X POST http://localhost:3000/api/v1/projects/<PROJECT_ID>/normalizations \
  -H "content-type: application/json" \
  -d '{
...
### 4A. Start analysis
```bash
curl -X POST http://localhost:3000/api/v1/projects/<PROJECT_ID>/analyses \
  -H "content-type: application/json" \
  -d '{
...
```bash
curl http://localhost:3000/api/v1/analyses/<ANALYSIS_ID>
```
Expected: `200` with `data.status` = `completed`.

### 4C. Start creative generation
```bash
curl -X POST http://localhost:3000/api/v1/projects/<PROJECT_ID>/creatives \
  -H "content-type: application/json" \
  -d '{
...
### 4D. Generate report
```bash
curl -X POST http://localhost:3000/api/v1/projects/<PROJECT_ID>/reports \
  -H "content-type: application/json" \
  -d '{
    "analysisId": "<ANALYSIS_ID>",
...
```bash
curl http://localhost:3000/api/v1/projects/<PROJECT_ID>/jobs
```
...
```bash
curl -O http://localhost:3000/api/v1/reports/<REPORT_ID>/download
```
